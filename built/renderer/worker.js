...
function saveProgress(params) {
    return __awaiter(this, void 0, void 0, function* () {
        ...
        const result = yield excel.saveNonInteractive(params, data);
        console.log(result);
 ...
 * @param {string} series
 */
function getCurItemNum(series) {
    return __awaiter(this, void 0, void 0, function* () {
        const maximo = new Maximo();
        let num;
        try {
            num = yield maximo.getCurItemNumber(series);
...
function update(e) {
    return __awaiter(this, void 0, void 0, function* () {
        ...
        if (updateType === 'manufacturer') {
            const data = yield excel.getManufactures();
            db.populateManufacturers(data);
        }
        else if (updateType === 'abbreviations') {
            const data = yield excel.getAbbreviations();
            db.populateAbbreviations(data);
        }
    });
}
function refreshTranslations(filePath) {
    return __awaiter(this, void 0, void 0, function* () {
        // load updated translation list from excel file
        const excel = new Spreadsheet(filePath);
        const data = yield excel.getTranslations();
        const db = new TranslationDatabase();
        const result = db.refreshData(data);
        postMessage(['result', result]);
    });
}
function compareObservLists(data, savePath, jobTaskPath) {
    return __awaiter(this, void 0, void 0, function* () {
        ...
        yield excelW.saveObserListChanges({
...
function checkUser(credentials = {}) {
    return __awaiter(this, void 0, void 0, function* () {
        ...
        const currStatus = yield maximo.checkLogin(credentials === null || credentials === void 0 ? void 0 : credentials.userid, credentials === null || credentials === void 0 ? void 0 : credentials.password);
 ...
 */
function checkItemCache(version) {
    return __awaiter(this, void 0, void 0, function* () {
        ...
        if (!(yield shareDB.checkVersion(version))) {
            db.createTables();
            const filePath = path.join(require('path').resolve(__dirname).replace('src\\renderer', 'assets'), 'item_information.xlsx');
            const excel = new ExcelReader(filePath);
            postMessage(['debug', `10%: Loading cache data from file`]);
            db.clearItemCache();
            const data = yield excel.getItemCache();
            ...
            const manu = yield excel.getManufactures();
            const abbr = yield excel.getAbbreviations();
        ...
        const newInventory = yield maximo.getNewInventory(curVersion);
        if (newInventory) {
        ...
        let newItems = yield maximo.getNewItems(curVersion);
        if (newItems) {
        ...
        newItems = yield maximo.getNewManufacturers(curVersion);
        if (newItems) {
 ...
 */
function uploadAllItems(items, doUpdate = false) {
    return __awaiter(this, void 0, void 0, function* () {
        const maximo = new Maximo();
        ...
        for (const item of items) {
            ...
            try {
                ...
                if (item.itemnumber === 0 || item.itemnumber.length != 7) {
                    ...
                    num = yield maximo.getCurItemNumber(item.series);
                ...
                if (doUpdate)
                    postMessage(['update', 'fail', rowIndex]);
                postMessage(['fail', err]);
                continue;
            }
            try {
                const result = yield maximo.uploadToMaximo(item);
                if (!result) {
                    if (doUpdate)
                        postMessage(['update', 'fail', rowIndex]);
                    ...
                    if (doUpdate)
                        postMessage(['update', 'success', rowIndex]);
            ...
            if (item.storeroomname != '' || item.siteID != '' || item.cataloguenum != '' || item.vendorname != '') {
                try {
                    const result = yield maximo.uploadToInventory(item);
    ...
    return __awaiter(this, void 0, void 0, function* () {
        try {
            ...
            for (let i = 0; i < images.length; i++) {
                ...
                const data = yield maximo.uploadImageToMaximo(img);
                const result = data[0];
