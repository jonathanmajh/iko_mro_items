...
class AssetTranslateDescription {
    translate(params) {
        return __awaiter(this, void 0, void 0, function* () {
            ...
            yield wb.csv.writeFile(path.join(new_folder, 'location.csv'), options);
            // write assets sheet
            ...
            yield wb.csv.writeFile(path.join(new_folder, 'asset.csv'), options);
            //get jobplan information
            excel = new TransDB(params.wb_pms);
            const jps = yield excel.ReadColumns(['jpnum', 'description', 'siteid']);
            wb = new Exceljs.Workbook();
            ...
            yield wb.csv.writeFile(path.join(new_folder, 'jobplans.csv'), options);
            const pms = yield excel.ReadColumns(['pmnum', 'description', 'siteid']);
            wb = new Exceljs.Workbook();
            ...
            yield wb.csv.writeFile(path.join(new_folder, 'pms.csv'), options);
            yield logbook.csv.writeFile(path.join(new_folder, 'logs.csv'));
    ...
    translateDescription(info, lookups, translations, logsheet, thing, lang_code) {
        ...
        if (thing.type === 'sjp') {
            ...
            if (thing.worktype.indexOf('lc') != -1) {
                ...
                translated = `${translated}${temp}`;
                temp = translations[`${lang_code}${(_a = thing.suffix) === null || _a === void 0 ? void 0 : _a.replace(/\s/g, " ").toLowerCase()}`];
                if (temp) {
    ...
    NumParser(info, specs, lookups) {
        ...
        if (text.length < 10 && sjp_type[text.slice(5)]) {
            ...
            if (match != null) {
                ...
                specs['suffix'] = (_a = info.description.match(lc_item)) === null || _a === void 0 ? void 0 : _a[0];
            }
